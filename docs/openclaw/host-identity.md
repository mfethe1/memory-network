...
An OpenClaw host is a Linux or macOS machine that runs the host daemon and
connects outbound to the fleet broker and controller. Host identity is stable
across reboots and hostname changes, but it is not a human identity and it is
not an Agent Run identity.

Lenny and Rosie are deployment aliases. The initial M1 deployment path uses the
Linux `systemd` installer for Lenny and the macOS `launchd` installer for Rosie;
operator steps live in [linux-macos-host-enrollment.md](linux-macos-host-enrollment.md).

## Identity Fields

The controller stores one host record per enrolled machine:

```text
host_id                     Stable controller-assigned ID, for example oclh_01hz...
display_name                Human label, not used for authorization
host_aliases                Human routing labels such as lenny or rosie
environment                 dev, staging, or prod
owner                       Person or team responsible for the machine
machine_fingerprint_hash    Salted hash of local machine identifiers
install_nonce_hash          Hash of the OpenClaw install nonce
os_family                   linux or darwin
os_version                  Distro/kernel or macOS version/build
...
secret_storage              OS keychain, protected file, or service account target names only
created_at                  Enrollment time
last_seen_at                Last heartbeat time
status                      pending, active, disabled, or rotated
```

Never use hostname, local username, MAC address, or IP address as the
...
`host_aliases` are routing labels only. They are not host IDs, authorization
keys, Telegram identities, local usernames, or NATS credential names.
...
3. The platform installer starts under the dedicated OpenClaw service account or
   user service context, collects local identity fields, and creates an install
   nonce.
...
6. The host stores the NATS credential and controller token using OS-appropriate
   secret storage or protected service-account files, then deletes any transient
   enrollment material.
...
Store host secrets with OS-appropriate protection. The first Linux/macOS M1
slice may use protected files owned by the service account, but production
enrollment should prefer the platform keychain where practical.
...
- Prefer entries or files owned by the dedicated OpenClaw service account.
- If a file must exist for a NATS client library, restrict it to the service
  account and admin users only.
...
SSH is allowed only for admin and break-glass operations over a private network
such as Tailscale:
