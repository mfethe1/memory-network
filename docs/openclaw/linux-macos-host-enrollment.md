...
For the canonical broker cutover, both hosts must use host-reachable URLs for
the same authenticated broker that the Railway controller uses. The literal
URLs may differ when the controller uses a Railway-internal NATS address and
hosts use a public TCP endpoint. Do not rely on Rosie-local `--nats-conf`
defaults for that cutover.
