# Canonical identity model (authoritative)
...
file path and line numbers (so moving a function between files keeps its
UID). It **changes** whenever `canonical_name`, `signature_norm`,
`container_uid`, `kind`, or `language` changes. Historical passages below
that call this "durable identity" or "stable across ... refactors" predate
this clarification; treat this section as authoritative. Explicit refactors
can migrate identity in place via `code_index update --rename-map PATH`
(JSON array of `{"old": canonical, "new": canonical}` pairs); that path
preserves `symbol_pk` so every FK edge in `relations`, `occurrences`,
`chunks.primary_symbol_pk`, `test_edges`, and `chunk_symbols` survives.
`doctor --json` surfaces the same fact under an `identity_model` block.
...
Prioritize:
1. Stable symbol identity across edits (see "Canonical identity model" at top — renames/refactors require `--rename-map`)
